...
const css = `
const PAYMENTS_LAYOUT_COMPACT_V3_CSS = String.raw\`
...
if (!source.includes("PAYMENTS_LAYOUT_COMPACT_V3_CSS")) {
...
if (!source.includes('<style>{PAYMENTS_LAYOUT_COMPACT_V3_CSS}</style>')) {
  ...
  source = source.replace(styleMarker, `${styleMarker}\n      <style>{PAYMENTS_LAYOUT_COMPACT_V3_CSS}</style>`);
}

fs.writeFileSync(paymentsPath, source);
console.log("Payments V3 mobile layout correction applied.");
