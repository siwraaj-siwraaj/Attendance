...
source = source.replace(
  ...
  '<h1 className="payments-title text-xl font-bold text-white">Payments</h1>',
);
source = source.replace(
  '<div className="relative mb-3" ref={dropdownRef}>',
  '<div className="relative mb-3 payments-contract-wrapper" ref={dropdownRef}>',
);
source = source.replace(
  ...
  'className="payments-contract-dropdown absolute left-0 top-[calc(100%+8px)] z-[250] w-[min(360px,90vw)] max-h-[60vh] flex flex-col overflow-hidden rounded-2xl border border-white/10 shadow-2xl"',
);

if (!source.includes("PAYMENTS_LAYOUT_COMPACT_V2")) {
  const css = String.raw`
const PAYMENTS_LAYOUT_COMPACT_V2 = true;
const PAYMENTS_LAYOUT_COMPACT_CSS = String.raw\`
  ...
  .payments-page-shell .payments-title {
    order: 1;
    min-width: 0;
    white-space: nowrap;
  ...
  .payments-page-shell .payments-contract-wrapper [data-ocid="payments.contract_select_trigger"] {
    width: 100%;
    min-width: 0;
    min-height: 44px;
    height: 44px;
  ...
  .payments-page-shell .payments-contract-wrapper [data-ocid="payments.contract_select_trigger"] > span:first-child {
    min-width: 0;
  }
  .payments-page-shell .payments-contract-dropdown {
  ...
  .payments-page-shell [data-ocid="payments.calculate_button"] {
    order: 3;
    min-width: 104px;
    min-height: 44px;
    height: 44px;
  ...
  .payments-page-shell .payments-more-control {
    order: 4;
    min-width: 68px;
    position: relative;
  }
  .payments-page-shell .payments-more-button {
    min-width: 68px;
    min-height: 44px;
    height: 44px;
    padding: 0 8px;
  ...
  .payments-page-shell .payments-more-menu {
    position: absolute !important;
    right: 0;
    top: calc(100% + 8px);
    z-index: 300;
  ...
  @media (max-width: 430px) {
    .payments-page-shell .payments-topbar {
      grid-template-columns: 82px minmax(88px,1fr) 104px 68px;
      gap: 5px;
      padding-left: 10px;
      padding-right: 10px;
    }
  ...
  @media (max-width: 370px) {
    .payments-page-shell .payments-topbar {
      grid-template-columns: 76px minmax(76px,1fr) 96px 62px;
      gap: 4px;
      padding-left: 8px;
      padding-right: 8px;
    }
    ...
    .payments-page-shell .payments-more-control,
    .payments-page-shell .payments-more-button { min-width: 62px; }
  }
\`;
`;
...
console.log("Payments layout V2 fixed: one-row header, non-reflowing contract dropdown, anchored More menu.");
