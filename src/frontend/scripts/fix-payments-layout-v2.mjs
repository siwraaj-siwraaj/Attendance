...
if (!source.includes("PAYMENTS_LAYOUT_COMPACT_V2_CSS")) {
  const css = `
const PAYMENTS_LAYOUT_COMPACT_V2_CSS = ${JSON.stringify(`
.payments-page-shell .payments-topbar{position:relative;display:grid;grid-template-columns:auto minmax(90px,1fr) auto auto;align-items:center;gap:6px;min-height:64px;padding-top:10px;padding-bottom:10px;overflow:visible}
...
@media (max-width:430px){.payments-page-shell .payments-topbar{grid-template-columns:82px minmax(88px,1fr) 104px 68px;gap:5px;padding-left:10px;padding-right:10px}.payments-page-shell .payments-title{font-size:20px}.payments-page-shell .payments-contract-wrapper [data-ocid="payments.contract_select_trigger"]{font-size:11px}.payments-page-shell .payments-more-button>span:first-child{display:none}}
@media (max-width:370px){.payments-page-shell .payments-topbar{grid-template-columns:76px minmax(76px,1fr) 96px 62px;gap:4px;padding-left:8px;padding-right:8px}.payments-page-shell .payments-title{font-size:18px}.payments-page-shell [data-ocid="payments.calculate_button"]{min-width:96px}.payments-page-shell .payments-more-control,.payments-page-shell .payments-more-button{min-width:62px}}
`)}`;
...
console.log("Payments layout V2 fixed.");
