...
export function createSupabaseActor() {
  return {
    ...
    async getCallerStatus() {
      ...
      const rows = await rest("profiles", { query: `?select=status&id=eq.${data.user.id}` });
      return rows?.[0]?.status ?? "pending";
    },
    async getCallerRole(): Promise<Role | null> {
      const { data } = await supabase.auth.getUser();
      if (!data.user) return null;
      const rows = await rest("profiles", { query: `?select=role&id=eq.${data.user.id}` });
      return rows?.[0]?.role ?? null;
    },
    async getCallerRoles(): Promise<Role[]> {
      const { data } = await supabase.auth.getUser();
      if (!data.user) return [];
      const rows = await rest("profiles", { query: `?select=role,roles&id=eq.${data.user.id}` });
    ...
    async deleteContract(id: bigint) {
      try { await rest("contracts", { method: "DELETE", query: `?id=eq.${id}` }); return ok(true); } catch (e: any) { return err(e.message); }
    },
    async markContractSettled(id: bigint, settled: boolean) {
      try { await rest("contracts", { method: "PATCH", query: `?id=eq.${id}`, body: { settled } }); return ok(true); } catch (e: any) { return err(e.message); }
    },
    async addWorkColumn(contractId: bigint, name: string, workType: string) {
      try {
        const normalizedType = String(workType ?? "").trim().toLowerCase();
        const fallbackNames: Record<string, string> = { bed: "Bed", paper: "Paper", mesh: "Mesh" };
        const baseName = fallbackNames[normalizedType] || String(workType).trim();
        if (!baseName) return err("Work type is required");
        const existing = await rest("work_columns", { query: `?select=name&contract_id=eq.${encodeURIComponent(contractId.toString())}&work_type=eq.${encodeURIComponent(normalizedType)}` });
        let maxNumber = 0;
        for (const column of existing) {
          const existingName = String(column.name ?? "").trim();
          if (existingName === baseName) { maxNumber = Math.max(maxNumber, 1); continue; }
          const match = existingName.match(new RegExp(`^${baseName}(\\d+)$`));
    ...
    async addAdvance(contractId: bigint, labourId: bigint, amount: number, note: string) {
      try {
        const rows = await rest("advances", { method: "POST", headers: { Prefer: "return=representation" }, body: { contract_id: contractId.toString(), labour_id: labourId.toString(), amount, note, cleared: false } });
        return ok(mapAdvance(rows[0]));
      } catch (e: any) { return err(e.message); }
    },
    async updateAdvance(id: bigint, amount: number, note: string, cleared: boolean) {
      try {
        const rows = await rest("advances", { method: "PATCH", query: `?id=eq.${id}`, body: { amount, note, cleared } });
        return ok(mapAdvance(rows[0]));
      } catch (e: any) { return err(e.message); }
    },
    async deleteAdvance(id: bigint) {
      try { await rest("advances", { method: "DELETE", query: `?id=eq.${id}` }); return ok(true); } catch (e: any) { return err(e.message); }
    },
    async listUsers() { const data = await functionCall("rossie-admin", { action: "list" }); return data.users ?? []; },
    async createUser(username: string, password: string, role: Role | Role[]) {
      try { const data = await functionCall("rossie-admin", { action: "create", username, password, roles: Array.isArray(role) ? role : [role] }); return Boolean(data?.created); } catch { return false; }
    },
    ...
    async exportData() {
      ...
      const contractColumns = columns.reduce((result: any, column: any) => {
        ...
        if (!result[contractId]) result[contractId] = [];
        result[contractId].push({ id: column.id, contractId: BigInt(column.contract_id), name: column.name, workType: column.work_type });
        return result;
      }, {});
    ...
    async execute(_queryJson: string) { throw new Error("OQL is not available in the Supabase backend."); },
    async schema() { return "Supabase backend"; },
  };
}
