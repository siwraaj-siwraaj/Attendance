...
export default function AdminPanel() {
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto pb-safe" data-ocid="admin_panel">
      ...
      <Dialog open={deleteUserTarget!==null} onOpenChange={open=>{if(!open)setDeleteUserTarget(null)}}><DialogContent className="glass-dialog border-border max-w-md overflow-hidden p-0" data-ocid="admin_panel.delete_user_modal"><div className="border-b border-white/[0.07] bg-gradient-to-br from-red-500/15 via-transparent to-transparent p-5"><div className="flex items-start gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-red-500/15 text-red-300 ring-1 ring-red-400/20"><Trash2 size={20}/></div><div className="min-w-0 flex-1"><DialogTitle className="font-display text-white">Delete user account</DialogTitle><DialogDescription className="mt-1">Permanent removal from Rossie.</DialogDescription></div></div></div><div className="space-y-4 p-5"><div className="rounded-2xl border border-white/[0.07] bg-white/[0.035] p-3.5"><p className="text-sm font-semibold text-white">{deleteUserTarget?.name || "User account"}</p><p className="mt-1 text-[11px] text-white/40">{deleteUserTarget?.username}</p></div><div className="rounded-2xl border border-red-400/15 bg-red-500/[0.07] p-3.5 text-xs leading-5 text-red-100/75">This permanently removes the login account and access. The labour record is not deleted by this action.</div><DialogFooter className="gap-2 sm:gap-2"><Button variant="outline" onClick={()=>setDeleteUserTarget(null)} data-ocid="admin_panel.delete_user_cancel">Cancel</Button><Button variant="destructive" disabled={deleteUserMutation.isPending} onClick={()=>{if(deleteUserTarget)deleteUserMutation.mutate(deleteUserTarget.username,{onSuccess:()=>setDeleteUserTarget(null),onError:e=>void showAppNotification("Account deletion failed",e instanceof Error?e.message:"Could not delete account")})}} data-ocid="admin_panel.delete_user_confirm">{deleteUserMutation.isPending?"Deleting…":"Delete account"}</Button></DialogFooter></div></DialogContent></Dialog>
